...
function speak() {
  setTimeout(() => playByText("en-US", "Hello,world  "), 600);
}

let open_response_response;
...
async function conversationAssistantAddAdd(response) {
...
async function open_ai() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      conversationAssistantAddAdd(message);
